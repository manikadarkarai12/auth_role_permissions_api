...
router.post('/', passport.authenticate('jwt', {
    ...
    helper.checkPermission(req.user.role_id, 'permission_create').then((rolePerm) => {
...
router.get('/', passport.authenticate('jwt', {
    ...
    helper.checkPermission(req.user.role_id, 'permission_list').then((rolePerm) => {
        Permission
            .findAll()
...
router.get('/:id', passport.authenticate('jwt', {
    ...
    helper.checkPermission(req.user.role_id, 'permission_read').then((rolePerm) => {
    ...
    Permission
        ...
        .then((perm) => res.status(200).send(perm))
        .catch((error) => {
            res.status(400).send(error);
        });
});
...
router.put('/:id', passport.authenticate('jwt', {
    ...
    helper.checkPermission(req.user.role_id, 'permission_update').then((rolePerm) => {
...
router.delete('/:id', passport.authenticate('jwt', {
    ...
    helper.checkPermission(req.user.role_id, 'permission_delete').then((rolePerm) => {
        if (!req.params.id) {
            res.status(400).send({
